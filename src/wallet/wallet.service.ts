...
export class WalletService {
  ...
  async createDepositRequest(userId: string, dto: DepositRequestDto): Promise<DepositResponseDto> {
    ...
    try {
      ...
      console.log(`✅ Flutterwave payment initialized for deposit ${depositId}`);
      ...
      console.error('❌ Failed to initialize Flutterwave payment:', error);
  ...
  async createWithdrawRequest(userId: string, dto: WithdrawRequestDto): Promise<PayoutRequestResponseDto> {
    ...
    try {
      ...
      console.log(`✅ Flutterwave transfer initiated for payout ${payoutId}`);
      console.log(`💱 Conversion details: ${dto.fretiAmount} USD (FRETI) → ~${estimatedLocalAmount.toFixed(2)} ${localCurrency} (estimated)`);
