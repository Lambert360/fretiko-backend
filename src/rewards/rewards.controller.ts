...
export class RewardsController {
  ...
  async getRewardsBalance(@Request() req: any) {
    this.logger.log(`Getting rewards balance for user ${req.user.id}`);
    
    const balance = await this.rewardsService.getUserRewardsBalance(req.user.id);
  ...
  async getRewardsSummary(@Request() req: any) {
    this.logger.log(`Getting rewards summary for user ${req.user.id}`);
    
    const summary = await this.rewardsService.getUserRewardsSummary(req.user.id);
  ...
  async redeemRewards(@Request() req: any, @Body() body: RedeemRewardsDto) {
    this.logger.log(`User ${req.user.id} redeeming ${body.rewards_amount} rewards`);
  ...
  async reverseRedemption(@Request() req: any, @Body() body: ReverseRedemptionDto) {
    this.logger.log(`Reversing ${body.rewards_amount} rewards for user ${req.user.id}`);
  ...
  async calculatePeriodRewards(
    ...
    this.logger.log(`Calculating rewards for user ${req.user.id} period ${period}`);
    
    const result = await this.rewardsService.calculateMonthlyRewards(req.user.id, period);
  ...
  async creditPeriodRewards(
    ...
    this.logger.log(`Crediting rewards for user ${req.user.id} period ${period}`);
    
    const success = await this.rewardsService.creditMonthlyRewards(req.user.id, period);
  ...
  async getWalletDisplay(@Request() req: any) {
    try {
      this.logger.log(`Getting wallet display rewards for user ${req.user.id}`);
      ...
      this.logger.error(`Error getting wallet display rewards for user ${req.user.id}:`, error);
  ...
  async getCheckoutDisplay(@Request() req: any) {
    try {
      this.logger.log(`Getting checkout display rewards for user ${req.user.id}`);
      ...
      this.logger.error(`Error getting checkout display rewards for user ${req.user.id}:`, error);
      // Return safe defaults instead of throwing
      return {
        available_rewards: 0,
        display_available: this.rewardsService.fretiToRewardsDisplay(0),
