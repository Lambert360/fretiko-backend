-- Fix missing notification_settings table
-- This fixes the "relation 'notification_settings' does not exist" error
...
CREATE INDEX IF NOT EXISTS idx_notifications_user_unread ON public.notifications(user_id, is_read) WHERE is_read = FALSE AND is_deleted = FALSE;
...
CREATE INDEX IF NOT EXISTS idx_notification_settings_push_enabled ON public.notification_settings(push_enabled) WHERE push_enabled = TRUE;
...
DROP VIEW IF EXISTS public.user_notification_stats;
...
DROP VIEW IF EXISTS public.recent_notifications;
...
DROP VIEW IF EXISTS public.urgent_notifications;
CREATE VIEW public.urgent_notifications AS
SELECT *
FROM public.notifications
WHERE is_read = FALSE
  AND is_deleted = FALSE
...
CREATE POLICY "Users can view own receipts" ON public.notification_receipts
    FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert own receipts" ON public.notification_receipts;
CREATE POLICY "Users can insert own receipts" ON public.notification_receipts
    FOR INSERT WITH CHECK (auth.uid() = user_id);
...
DROP TRIGGER IF EXISTS trigger_create_notification_settings ON public.user_profiles;
...
WHERE NOT EXISTS (
    ...
    WHERE notification_settings.user_id = user_profiles.id
)
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.notification_receipts TO authenticated;
...
DO $$
BEGIN
    RAISE NOTICE 'Notification system tables and components have been created successfully!';
    RAISE NOTICE 'User creation should now work without notification_settings-related errors';
END $$;