-- Migration: Fix logistics application procedure completely
-- Date: 2026-04-05
-- Description: Completely drop and recreate stored procedure to ensure no ambiguity

-- Drop existing function completely (using exact signature from database)
DROP FUNCTION IF EXISTS create_logistics_application(character varying,text,character varying,character varying,character varying,character varying,character varying,text,text[],jsonb,jsonb,integer,integer,integer,jsonb,text[],text[],text[],text[]);

-- Recreate function with completely different approach
