...
-- Date: 2026-01-20
-- Description: 
--   Fix critical race condition in idempotency check
--   Industry standard: Acquire row locks BEFORE checking idempotency
--   This prevents concurrent requests from both passing the idempotency check
-- =====================================================
...
-- Ensure idempotency index exists (for performance)
...
-- ✅ STEP 1: Clean up existing duplicate transactions before creating unique index
-- This handles the case where duplicates were already created due to the race condition bug
DO $$
DECLARE
  duplicate_count INTEGER;
BEGIN
  -- Find and remove duplicate transactions, keeping only the first one (oldest)
  -- This fixes existing data that was corrupted by the race condition
  -- Note: This only cleans up ledger entries. Wallet balances may need separate reconciliation
...
-- ✅ STEP 2: Add unique index to prevent duplicates at database level (defense in depth)
-- This provides an additional layer of protection beyond application-level checks
-- Using CREATE UNIQUE INDEX instead of constraint to support partial indexes (WHERE clause)
