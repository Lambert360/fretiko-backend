-- Migration: Simplified ESCROW_REFUND fix - Version 158b
-- Description: Minimal fix to resolve "structure of query does not match function result type" error
-- This version only changes the RETURN QUERY statements to avoid migration conflicts

-- First, let's see what the current function looks like
-- \df process_wallet_transaction

-- Update only the RETURN QUERY statements with proper type casting
...
BEGIN
  ...
  -- 7. Return success - FIXED: Keep TEXT cast for NULL to avoid type ambiguity
  RETURN QUERY SELECT TRUE, v_transaction_id, v_new_available, v_new_escrow, v_new_pending, NULL::TEXT;

EXCEPTION
  WHEN OTHERS THEN
    -- Log error and return failure - SIMPLIFIED: Remove explicit type casting
    RETURN QUERY SELECT FALSE, NULL, v_current_available, v_current_escrow, v_current_pending, 
                   SQLERRM;
END;
$$;
