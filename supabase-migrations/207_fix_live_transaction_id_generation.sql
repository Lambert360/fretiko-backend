-- =====================================================
-- Migration: 207
-- Fix: two atomic RPCs never generate a valid UUID for
-- live_stream_transactions.id, causing every live product purchase and
-- every live service/portfolio booking to fail.
--
-- Bug 1 (create_live_product_order_atomic, from migration 199/205):
--   INSERT INTO live_stream_transactions (id, ...) VALUES (
--     p_live_transaction->>'id', ...
--   )
--   `->>'id'` extracts TEXT, and the client sends a non-UUID string like
--   "live_1789161803243_vyymvjlmn" (a JS-generated correlation id, not a
--   real UUID). Every other UUID column in that VALUES list has an
--   explicit ::UUID cast except this one, so Postgres fails with:
--     "column \"id\" is of type uuid but expression is of type text"
--   (and would still fail with a cast, since the value isn't a valid UUID).
--
-- Bug 2 (create_live_service_booking_atomic, from migration 200/206):
--   v_live_transaction_json := p_live_transaction - 'id' - ...
...
--   Dropping 'id' relies on the column's DEFAULT gen_random_uuid(), but
--   jsonb_populate_record does NOT apply column defaults for missing keys
--   -- it inserts an explicit NULL instead. This is the exact same class of
--   bug that migration 205 fixed for orders.id, just left over here for
--   live_stream_transactions.id. Result:
--     "null value in column \"id\" of relation \"live_stream_transactions\"
--      violates not-null constraint"
--
-- Fix: both functions now generate v_transaction_id := gen_random_uuid()
-- explicitly and use that for the id column, ignoring any client-supplied
-- 'id' value. All other logic is unchanged from 205/206.
-- =====================================================
...
BEGIN
  ...
  -- Generate the live_stream_transactions id explicitly. The client sends a
  -- correlation string (e.g. "live_170...") in p_live_transaction->>'id',
  -- which is NOT a valid UUID -- it must never be used for this column.
...
'Atomically creates a live product order, its line items, decrements live stock, redeems a gift card, holds wallet funds, creates an escrow record, and inserts a live_stream_transactions record (with a server-generated UUID id) in one Postgres transaction.';
...
-- =====================================================
-- Fix create_live_service_booking_atomic (used by both regular live
-- service bookings and portfolio bookings): generate the
-- live_stream_transactions id explicitly instead of relying on
-- jsonb_populate_record to apply the column default (it doesn't).
-- =====================================================
...
BEGIN
  ...
  -- Generate the live_stream_transactions id explicitly. jsonb_populate_record
  -- does NOT apply column defaults for keys missing from the JSON payload --
  -- it inserts NULL -- so 'id' must always be supplied here.
  v_live_transaction_id := gen_random_uuid();
...
'Atomically books a live service slot (or skips slot reservation for portfolio/unlisted service bookings), creates the service order, holds wallet funds, creates an escrow, and inserts a live stream transaction (with a server-generated UUID id) in one Postgres transaction.';

REVOKE ALL ON FUNCTION create_live_service_booking_atomic(UUID, JSONB, JSONB, JSONB, UUID, JSONB, JSONB, JSONB, NUMERIC, UUID, TEXT, BOOLEAN) FROM PUBLIC;
REVOKE ALL ON FUNCTION create_live_service_booking_atomic(UUID, JSONB, JSONB, JSONB, UUID, JSONB, JSONB, JSONB, NUMERIC, UUID, TEXT, BOOLEAN) FROM anon;
REVOKE ALL ON FUNCTION create_live_service_booking_atomic(UUID, JSONB, JSONB, JSONB, UUID, JSONB, JSONB, JSONB, NUMERIC, UUID, TEXT, BOOLEAN) FROM authenticated;
GRANT EXECUTE ON FUNCTION create_live_service_booking_atomic(UUID, JSONB, JSONB, JSONB, UUID, JSONB, JSONB, JSONB, NUMERIC, UUID, TEXT, BOOLEAN) TO service_role;
ALTER FUNCTION create_live_service_booking_atomic(UUID, JSONB, JSONB, JSONB, UUID, JSONB, JSONB, JSONB, NUMERIC, UUID, TEXT, BOOLEAN) SET search_path = public, pg_temp;

-- =====================================================
-- Verification (run manually after applying):
--
-- select proname, pg_get_function_arguments(oid) from pg_proc
-- where proname in ('create_live_product_order_atomic', 'create_live_service_booking_atomic');
-- =====================================================
