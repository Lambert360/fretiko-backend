...
-- Migration 210: Add current_item_id to the auction_summary view
--
-- Why:
--   Multi-item live auctions need to know the active item at fetch time.
--   The auction_summary view was originally created before auctions.current_item_id
--   existed, so findById() (which uses the view) returned undefined for
--   current_item_id. This caused live bids to be inserted with item_id = null
--   and the bid trigger to validate them as timed auction bids.

-- Postgres CREATE OR REPLACE VIEW cannot insert a column mid-list, so drop first.
-- Nothing else depends on this view (verified against migrations).
...
COMMENT ON VIEW public.auction_summary IS 'Auction summary with current_item_id added for live multi-item auctions';
