-- =====================================================
-- FIX RELEASE_ESCROW_ATOMIC RECORD ASSIGNMENT ERROR
-- Migration: 154
-- Date: 2026-01-22
-- Description: 
--   Fix PostgreSQL RECORD assignment error in release_escrow_atomic function.
--   Replace v_order RECORD with individual variables to avoid "record not assigned yet" error.
-- =====================================================

-- Drop and recreate the function with fixed variable declarations
-- Drop with all possible signatures to ensure clean removal
...
-- Create the atomic escrow release function with fixed variable declarations
...
DECLARE
  ...
  v_released_at TIMESTAMP;
  v_release_reason TEXT;
  v_escrow_created_at TIMESTAMP;
  v_escrow_updated_at TIMESTAMP;
  v_order_id UUID;
...
Fixed: Replaced v_order RECORD with individual variables to avoid "record not assigned yet" error.';
