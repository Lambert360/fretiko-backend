...
-- =====================================================
-- UPDATE WALLET TRANSACTION RPC FOR GIFTS
-- Migration: 146
-- Date: 2025-01-20
-- Description: Add gift_purchase and gift_conversion support to process_wallet_transaction
-- =====================================================
...
BEGIN
  ...
  CASE p_transaction_type
    ...
    -- ✅ GIFT CONVERSION: Credit available (80% of gift value)
    WHEN 'gift_conversion' THEN
      v_available_delta := p_amount;
      v_new_available := v_current_available + p_amount;
      v_new_escrow := v_current_escrow;
      v_new_pending := v_current_pending;
      -- Gift conversion doesn't affect sales/earnings tracking
...
-- Add comment
COMMENT ON FUNCTION process_wallet_transaction IS 'Atomically process wallet transactions with balance updates and ledger entries. Now supports gift_purchase and gift_conversion transaction types.';
