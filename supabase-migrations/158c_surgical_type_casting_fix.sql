-- Migration: Surgical fix for ESCROW_REFUND type casting - Version 158c
-- Description: Fix only the critical type casting issues causing "structure of query does not match function result type"
-- This is a minimal surgical fix that preserves all existing logic
...
BEGIN
  ...
  -- Check if wallet exists - SURGICAL FIX: Add explicit type casting
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    -- SURGICAL FIX: Add explicit type casting
  ...
  -- 7. Return success - SURGICAL FIX: Add explicit type casting
  RETURN QUERY SELECT TRUE::BOOLEAN, v_transaction_id, v_new_available, v_new_escrow, v_new_pending, NULL::TEXT;

EXCEPTION
  WHEN OTHERS THEN
    -- Log error and return failure - SURGICAL FIX: Add explicit type casting
    RETURN QUERY SELECT FALSE::BOOLEAN, NULL::UUID, v_current_available, v_current_escrow, v_current_pending, 
                   SQLERRM::TEXT;
END;
$$;
