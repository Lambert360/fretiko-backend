-- Migration: Create Rewards System (FIXED VERSION)
-- Date: 2025-09-02
...
-- Rewards configuration table (for system settings) - FIXED VERSION
CREATE TABLE rewards_config (
    ...
    -- NO PROBLEMATIC CONSTRAINT HERE
);
...
DROP TRIGGER IF EXISTS update_rewards_balances_trigger ON rewards_transactions;
...
DROP TRIGGER IF EXISTS create_user_rewards_balance_trigger ON user_profiles;
...
DROP TRIGGER IF EXISTS update_rewards_calculations_updated_at ON rewards_calculations;
...
-- Insert default configuration - THIS WILL WORK NOW
