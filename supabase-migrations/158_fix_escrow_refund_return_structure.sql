-- Migration: Fix ESCROW_REFUND return structure mismatch
-- Description: Fixes the "structure of query does not match function result type" error
-- Bug: The RETURN QUERY statements were not matching the declared RETURNS TABLE structure
-- Impact: ESCROW_REFUND transactions were failing with 500 errors
-- Version: 158

-- Drop and recreate the process_wallet_transaction function with correct RETURN QUERY structure
...
BEGIN
  ...
  -- 7. Return success - FIXED: Explicit type casting to match RETURNS TABLE structure
  RETURN QUERY SELECT TRUE::BOOLEAN, v_transaction_id, v_new_available, v_new_escrow, v_new_pending, NULL::TEXT;

EXCEPTION
  WHEN OTHERS THEN
    -- Log error and return failure - FIXED: Explicit type casting
    RETURN QUERY SELECT FALSE::BOOLEAN, NULL::UUID, v_current_available, v_current_escrow, v_current_pending, 
                   SQLERRM::TEXT;
END;
$$;
...
COMMENT ON FUNCTION process_wallet_transaction IS 'Fixed RETURN QUERY structure with explicit type casting to resolve "structure of query does not match function result type" error (migration 158)';
